...
| -------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `filter` | Оставить/отбросить протоколы и номера AS; нормализовать параметры вида `insecure=1` | `protocols` / `exclude_protocols`: списки или null (= все); `asns` / `exclude_asns`: списки номеров AS или null — сверка с сохранённым `geo_asn`, allowlist отбрасывает прокси без определённого ASN (работают написания и `"24940"`, и `"AS24940"`); `normalize_params`: по умолчанию `true` |
