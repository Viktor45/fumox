...
которая всегда отдает все текущие живые прокси в формате `url_list` — ее
можно вставить в клиент или использовать как источник в другом fumox. Ссылка
несет случайный токен, сгенерированный при первом запуске; «Скачать url_list»
сохраняет ответ файлом, а «Перевыпустить ссылку» меняет токен, если ссылка
утекла (старая ссылка перестает работать сразу). Выдача начинается с блока
