...
3. **Переменные окружения**: `FUMOX_СЕКЦИЯ__КЛЮЧ`, где двойное подчеркивание
...
| `secure_cookies`     | `false`            | Добавлять `; Secure` к cookie сессии; включите, когда панель доступна через HTTPS reverse-proxy. На plain-HTTP оставляйте `false` — браузер молча отбрасывает `Secure`-cookie на `http://`, и со стороны выглядит как успешный вход (`admin logged in` в логах) с вечной петлёй редиректов на `/admin/login`. Переопределяется per-deploy через `FUMOX_ADMIN__SECURE_COOKIES=true` в `.env` (ENV приоритетнее файла). |
...
переименование; `host` и `port` переписывают адрес прокси, а `param:КЛЮЧ` затрагивает
