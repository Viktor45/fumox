...
- meow-rs не публикует официального Docker-образа, поэтому стек собирает
  маленькую обёртку (`docker/meow/Dockerfile`) вокруг релизного бинарника —
  либо берёт готовую из GHCR (см. Вариант B). Её REST API (порт 9090)
...
- **Одноразовый дымный стенд:** `scripts/smoke-up.sh` поднимает тот же стек
  ...
  образов (`ghcr.io/viktor45/fumox:latest`, `fumox-meow:local`), так что
  сборка стенда заодно обновляет образы и для основного.
...
`main` и на теги `v*`: `ghcr.io/viktor45/fumox`. Теги: push в `main` — `main` и
`sha-<короткий хеш>`; тег `v0.2.0` — дополнительно `0.2.0`, `0.2` и `latest`.
...
Обёртка meow-rs (`docker/meow/Dockerfile`) тоже есть в GHCR —
...
Варианта A может использовать готовый образ вместо локальной сборки:
замените у сервиса `meow` секцию `build:` на
`image: ghcr.io/viktor45/fumox-meow:<тег>` (переменная `.env` `MEOW_VERSION`
в этом случае ни на что не влияет — она выбирает релиз только при локальной
сборке).
...
| `fail_limit`                 | `3`                 | Неудач подряд до карантина (T1 и T2 — один счётчик)                     |
...
| `backoff_max_secs`     | `900`                                                                                                 | Потолок отступа T2 (15 мин) — мёртвый meow-rs никогда не принимается за мёртвые прокси                                                                                                                                                                                                                                                           |
...
4. **T2** — настоящие туннельные проверки через meow-rs: probe пишет
   ...
   списку). В выборку попадают `alive`-прокси, а также ещё не проверенные
...
`unknown`-hysteria2 сразу попадает в T2-выборку и дальше живёт обычной
машиной состояний (счётчик неудач общий с T1). **tuic и mieru непроверяемы**
...
| ------------------------------------ | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `[probe].fail_limit`                 | `3`                 | Неудач проверок подряд (T1 и T2 — один счётчик) до `quarantine`                                                                                      |
