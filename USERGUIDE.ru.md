...
- **Removed — не удаление.** Удаленные прокси остаются в базе (кнопка *Purge
  removed* в админке чистит их физически). `removed` — терминальный статус:
  если источник снова отдает такой прокси, его состояние **не** сбрасывается —
...
- **Контроль роста.** Probe автоматически чистит историю: `probe_results`
  ...
  Удаленные прокси накапливаются, пока вы не нажмете *Purge removed* в
  админке.
