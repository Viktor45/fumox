...
**Fumox** — легкий сервис агрегации прокси-подписок, написанный на Rust. Вы
...
Fumox — это Cargo-workspace из трех крейтов и одного внешнего помощника:
...
образе **оба** бинарника: сервер — команда по умолчанию, probe —
...
# Probe (тот же образ, те же тома; fumox-probe — аргумент команды,
...
5. **Наблюдайте вживую.** Дашборд показывает счётчики источников/прокси,
   ...
   умолчанию 10); браузер *Прокси* — все узлы со статусом, страной и
   задержкой; страница *Probe* — пульс демона проверок и очередь
...
исключаются — «только эти страны» означают подтвержденные факты, а не «все,
...
| Часть источников временно недоступна (ошибки `network`, `http_server`)                             | `200` + последние хорошие данные упавших источников + свежие остальных; заголовок `X-Fumox-Stale: true`. Stale отдается столько, сколько нужно — health-фильтр продолжает вычищать мертвые прокси, так что выдача «самолечится». |
...
2. **TOML-файл** — путь выбирается по приоритету: флаг `--config / -c` →
   ...
   заданный флагом или переменной, обязан существовать (опечатка в пути —
   громкая ошибка старта, а не тихий отход к дефолтам); отсутствует
...
| `fail_limit`                 | `3`                 | Неудач подряд до карантина (T1 и T2 — один счетчик)                     |
...
| `backoff_initial_secs` | `60`                                                                                                  | Начальный отступ T2, когда meow-rs недоступен (удваивается с каждым подряд идущим провалом)                                                                                                                                                                                                                                                      |
| `backoff_max_secs`     | `900`                                                                                                 | Потолок отступа T2 (15 мин) — мертвый meow-rs никогда не принимается за мертвые прокси                                                                                                                                                                                                                                                           |
...
значения: при инжесте — пайплайном самого источника (подходящий прокси не
...
все — фетч журналируется как parse-ошибка, база не тронута) и на отдаче
...
2. **Приоритетная очередь** — только что добавленные прокси, которые сервер
   ...
   на обновление); дренаж идет первыми самыми свежими, взятые записи удаляются
...
4. **T2** — настоящие туннельные проверки через meow-rs: probe пишет
   ...
   Порядок выборки — по давности, а не случайно: первыми идут прокси, у
...
- **Лестница повторных проверок.** После провала второго шанса прокси проходит
  ...
  удаляет прокси сразу после провала второго шанса. С дефолтами путь
  «карантин → removed» занимает ~13 ч 45 мин – 17 ч 45 мин.
- **Removed — не удаление.** Удаленные прокси остаются в базе (кнопка
  *«Стереть удалённые»* в админке чистит их физически). `removed` —
...
- **Исчезновение из источника не retiring живой прокси (alive-linger).**
  ...
  **при включенном** `[ingest].drop_gate = true` в конфиге (тогда правило,
...
| ------------------------------------ | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `[probe].fail_limit`                 | `3`                 | Неудач проверок подряд (T1 и T2 — один счетчик) до `quarantine`                                                                                      |
...
| `[probe].allow_private_targets`      | `false`             | Когда `false`, probe отказывается соединяться с loopback / link-local / RFC1918 / ULA адресами — защита от SSRF, если атакующий опубликует список прокси, указывающий на cloud-metadata (`169.254.169.254`) или приватную подсеть. Заблокированные попытки журналятся как `probe_results.error = "blocked by the private-address policy: <reason>"`, чтобы T2-очередь давности продвигалась дальше |
...
| `[meow].backoff_initial_secs`        | `60`                | Начальный отступ T2, когда meow-rs недоступен                                                                                                        |
| `[meow].backoff_max_secs`            | `900`               | Потолок отступа T2 (15 мин)                                                                                                                          |
