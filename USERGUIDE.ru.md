...
разобранные прокси с базой (новые → вставить, исчезнувшие → пометить removed,
вернувшиеся → воскресить) и журналирует каждую загрузку. Тем временем
...
2. **Добавьте источник.** *Источники → Новый*. Вставьте URL подписки, дайте
   ...
   Clash YAML), TTL кэша 1 час. Сохраните. Источник сразу будет загружен — вы
   увидите, сколько прокси найдено.
...
3. **Создайте профиль.** *Профили → Новый*. Дайте имя, отметьте источники,
   ...
   **токен доступа** (тогда клиенты должны его передавать — см. ниже).
...
| `GET /src/{id}` | Разобранные прокси **одного источника**. `{id}` — id источника или slug. |
...
Нет токена или токен неверный → `403 Forbidden`.
...
| ---------- | ------------------ | --------------------------------------------- |
| `uri_list` | `text/plain`       | По одной прокси-URI в строке                  |
...
(`X-Frame-Options: DENY` и т.д.), маскирование креденшелов прокси во всех
...
| -------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enabled`            | `true`             | Главный выключатель                                                                                                                                                          |
| `token`              | `"change-me"`      | Секрет входа. **Поменяйте его.** Пустое значение отключает панель (404)                                                                                                      |
...
| `test_url`     | `"http://www.gstatic.com/generate_204"` | URL, запрашиваемый через прокси для замеров задержки. В некоторых регионах заблокирован — поменяйте, если T2 не проходит нигде |
...
| `rename` | Переименование по regex, правила применяются по порядку                 | `match` (regex), `replace`, `flags` (например, `"i"`); по умолчанию `[]`                                                                    |
...
каждый цикл состоит из трёх проходов:

1. **Долги карантина** — вторые шансы и ступени повторных проверок, чей момент
   наступил;
2. **T1** — случайная выборка прямых проверок TCP-соединением (плюс
   TLS-рукопожатие, если протокол подразумевает TLS) по прокси
   `unknown`/`alive`;
3. **T2** — настоящие туннельные проверки `alive`-прокси через meow-rs: probe
...
stateDiagram-v2
    ...
    quarantine --> removed : второй шанс провален, затем 3 повторные провалены
    removed --> unknown : прокси снова появился в источнике (воскрешение)
```

Правила простым языком:

- **Успех всегда побеждает.** Любая успешная проверка переводит прокси в
  `alive` и обнуляет все счётчики неудач и расписания.
...
- **Removed — не удаление.** Удалённые прокси остаются в базе (кнопка *Purge
  removed* в админке чистит их физически). Если удалённый прокси снова
  появится в источнике, он воскресает со статусом `unknown` и чистым листом.
...
- **Бэкап конфигурации.** Используйте *Экспорт* в админке — он собирает все
  ...
  инстансе.
- **Временные метки** везде — Unix epoch в секундах, UTC.
...
| Нет флагов стран в именах                                    | Файл GeoLite2 `.mmdb` отсутствует в `[geo].db_dir`, либо `[geo].enabled = false`. Проверьте предупреждение о гео в стартовом логе.                    |
