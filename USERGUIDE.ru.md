...
платформа снабжена аттестацией происхождения сборки) в GHCR на каждый push в
`main` и на теги `v*`: `ghcr.io/viktor45/fumox`. Теги: push в `main` — `main`,
`sha-<короткий хеш>` и `latest`; тег `v0.2.0` — дополнительно `0.2.0` и `0.2`.
В образе **оба** бинарника: сервер — команда по умолчанию, probe —
