...
pub fn entry_to_outbound_named(entry: &ProxyEntry, tag: &str) -> Option<Value> {
    ...
    match entry.scheme {
        Scheme::Vless => {
            ...
            let reality = entry.param("security").unwrap_or_default() == "reality";
            if let Some(tls) = tls_object(entry, reality || wants_tls(entry), reality) {
                m.insert("tls".into(), tls);
            }
            if let Some(transport) = transport_object(entry) {
        ...
        Scheme::Vmess => {
            ...
            if entry.param("security").unwrap_or_default() == "tls"
                && let Some(tls) = tls_object(entry, true, false)
            {
                m.insert("tls".into(), tls);
            }
            if let Some(transport) = transport_object(entry) {
...
/// `reality` adds the REALITY sub-object with the entry's pbk/sid.
fn tls_object(entry: &ProxyEntry, enabled: bool, reality: bool) -> Option<Value> {
    ...
    if reality {
        ...
        if let Some(pbk) = non_empty_param(entry, "pbk") {
            r.insert("public_key".into(), Value::String(pbk.to_string()));
        }
        if let Some(sid) = non_empty_param(entry, "sid") {
            r.insert("short_id".into(), Value::String(sid.to_string()));
        }
...
/// WS/gRPC transport block from the `network`/`path`/`serviceName` params.
fn transport_object(entry: &ProxyEntry) -> Option<Value> {
    let mut t = Map::new();
    match entry.param("network").unwrap_or_default() {
        "ws" => {
            t.insert("type".into(), Value::String("ws".into()));
            if let Some(path) = non_empty_param(entry, "path") {
                t.insert("path".into(), Value::String(path.to_string()));
            }
        }
        "grpc" => {
            t.insert("type".into(), Value::String("grpc".into()));
            if let Some(service) = non_empty_param(entry, "serviceName") {
                t.insert("service_name".into(), Value::String(service.to_string()));
            }
        }
        _ => return None,
    }
